...
## Последнее обновление: 21.04.2026 — v1.5.0 (StateContext persistence)
...
**P-next-1: Smoke-тест Planner — ✅ пройден**

Два LLM-вызова на каждый запрос подтверждены в prod-логах:
- Planner: `llama-3.3-70b-versatile` (Groq p1, 646–2000ms)
- Executor: `llama-3.1-8b-instant` (Groq p1, 730–1400ms)

Fallback не триггерился. KB cron `+07:00` корректен (`BOT_TIMEZONE=Asia/Ho_Chi_Minh`).

---

**P-next-2: StateContext персистентность — ✅ пройден**

`app/handlers/messages.py` — runtime branch:
- `StateContext.from_defaults()` → `StateContext.from_db(db_user_state)`
- Lazy get-or-create `UserState` по `user_id` (FK `users.id`) внутри существующей сессии
- После `_vm.run()`: `db_user_state.context = run_result.state.to_db_context()` + `flush()`

Верифицировано на VPS: `UserState.context = {'fsm_state': 'idle'}` записан корректно после первого сообщения в режиме Runtime VM.
