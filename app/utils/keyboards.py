...
def get_skills_list_keyboard(
    skills: list[SkillInfo], page: int = 0, per_page: int = 5
) -> InlineKeyboardMarkup:
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text="⬅️ Назад", callback_data=f"skills:page:{page - 1}"
            )
        )
    if end < len(skills):
        nav_buttons.append(
            InlineKeyboardButton(
                text="Вперед ➡️", callback_data=f"skills:page:{page + 1}"
            )
        )
...
def get_skill_detail_keyboard(skill_name: str, source: str) -> InlineKeyboardMarkup:
    ...
    if source != "system":
        builder.button(text="🗑 Удалить", callback_data=f"skill:delete:{skill_name}")
...
def get_provider_detail_keyboard(
    provider_name: str, models: list, current_roles: dict
) -> InlineKeyboardMarkup:
    """
    ...
    builder.button(text="🔙 Назад", callback_data=f"providers:show:{provider_name}")
    builder.adjust(1)
    return builder.as_markup()


def get_provider_models_keyboard(
    provider_name: str, role: str, models: list, current_model: str
) -> InlineKeyboardMarkup:
    """
