...
class Settings(BaseSettings):
    ...
    VECTOR_STORE_PATH: str = Field(
        default="data/vector_store", description="Path to vector store directory"
    )
    ...
    SERPER_API_KEY: str = Field(
        default="", description="Serper.dev API key for Google search (recommended)"
    )
    SERPAPI_KEY: str = Field(
        default="", description="SerpAPI key for Google search (optional)"
    )
    BING_API_KEY: str = Field(default="", description="Bing Search API key (optional)")
    WEB_SEARCH_RESULTS: int = Field(
        default=3, description="Default number of search results"
    )

    # Multi-Provider Settings
    ENABLE_MULTI_PROVIDER: bool = Field(
        default=True, description="Enable automatic fallback between providers"
    )
    MAX_PROVIDER_RETRIES: int = Field(
        default=3, description="Maximum retries per provider"
    )
    ...
    SKILLBOT_MODEL: str = Field(
        default="llama-3.1-8b-instant", description="Model for SkillBot (code/skills)"
    )
    ...
    FASTBOT_SYSTEM_PROMPT: str = Field(
        default="You are a fast assistant...", description="System prompt for FastBot"
    )
    PLANBOT_SYSTEM_PROMPT: str = Field(
        default="You are a planning agent...", description="System prompt for PlanBot"
    )
    SKILLBOT_SYSTEM_PROMPT: str = Field(
        default="You manage skills...", description="System prompt for SkillBot"
    )
    ...
    MAX_SKILL_FILE_SIZE: int = Field(
        default=1048576, description="Max skill file size in bytes"
    )
    ...
    ENCRYPTION_KEY: str = Field(
        default="", description="Fernet key for encrypting provider API keys in DB"
    )
    ...
    MAX_VOICE_DURATION: int = Field(
        default=300, description="Maximum voice message duration in seconds"
    )
    WHISPER_MODEL: str = Field(
        default="whisper-large-v3", description="Whisper model for transcription"
    )
    ENABLE_VOICE_IN_GROUPS: bool = Field(
        default=True, description="Enable voice processing in groups"
    )
