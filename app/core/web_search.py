...
class WebSearchClient:
    ...
    async def search(
        self, query: str, num_results: int = 5, provider: Optional[str] = None
    ) -> List[SearchResult]:
        """
        Perform web search
    ...
    async def _search_serper(
        ...
        async with httpx.AsyncClient() as client:
            ...
            logger.info(
                f"Serper search returned {len(results)} results for: {query[:50]}..."
            )
            return results
    ...
    async def _search_serpapi(
        ...
        async with httpx.AsyncClient() as client:
            ...
            logger.info(
                f"SerpAPI search returned {len(results)} results for: {query[:50]}..."
            )
            return results
    ...
    async def _search_bing(
        ...
        async with httpx.AsyncClient() as client:
            ...
            logger.info(
                f"Bing search returned {len(results)} results for: {query[:50]}..."
            )
            return results
    ...
    async def _search_duckduckgo(
        ...
        async with httpx.AsyncClient() as client:
            ...
            logger.info(
                f"DuckDuckGo search returned {len(results)} results for: {query[:50]}..."
            )
            return results

    def format_results(
        self, results: List[SearchResult], max_length: int = 2000
    ) -> str:
