...
from app.handlers.commands import check_access_callback
import logging
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            vector_id = await vector_memory.add_memory(
                ...
                metadata={"source": "manual", "username": message.from_user.username},
            )
...
async def memory_search_query(message: Message, state: FSMContext):
    ...
    try:
        ...
        memories = await vector_memory.search_memories(
            query=query, user_id=message.from_user.id, n_results=10
        )
        ...
        if not memories:
            ...
            await message.answer(
                truncate_text(response), reply_markup=get_memory_menu_keyboard()
            )
...
async def memory_stats(callback: CallbackQuery):
    ...
    try:
        ...
        response += "<b>Векторное хранилище:</b>\n"
        ...
        await callback.message.edit_text(
            response, reply_markup=get_memory_menu_keyboard()
        )
