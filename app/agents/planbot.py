...
- ИМЕЕТЕ ДОСТУП К ПОИСКУ В ИНТЕРНЕТЕ для актуальной информации"""
    ...
    async def process_message(
        ...
        try:
            ...
            if self._needs_planning(message):
                return await self._process_with_planning(
            ...
            return (
                "⚠️ Произошла ошибка во время обработки. Пожалуйста, попробуйте снова."
            )
    ...
    async def _process_with_planning(
        ...
        if self._should_search_web(message):
            logger.info(
                f"PlanBot triggering web search for planning: {message[:50]}..."
            )
            try:
    ...
    async def _create_plan(
        self, goal: str, user_id: int, web_context: str = ""
    ) -> ExecutionPlan:
...
Держите шаги четкими и выполнимыми. Максимум 7 шагов."""
        ...
        response = await llm_client.chat_with_fallback(
            messages, model=settings.PLANNER_MODEL
        )
        ...
        try:
            ...
            steps = [
                PlanStep(number=1, description="Analyze and respond to the request")
            ]

        return ExecutionPlan(goal=goal, steps=steps)

    async def _execute_step(
        self, step: PlanStep, user_id: int, original_goal: str
    ) -> str:
...
Выполните этот шаг и предоставьте результат. Будьте краткими, но тщательными."""
    ...
    async def _generate_final_answer(
        self, goal: str, plan: ExecutionPlan, results: List[str], user_id: int
    ) -> str:
