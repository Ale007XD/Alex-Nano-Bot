...
7. ВАЖНО: Если в запросе пользователя есть блок [СИСТЕМНЫЙ КОНТЕКСТ], ты ОБЯЗАН передать этот блок целиком в параметр `system` для инструкции `call_llm`. Промпт в `prompt` оставляй без контекста (только сам вопрос).
